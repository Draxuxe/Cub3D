...
/*   Updated: 2022/11/07 17:29:56 by lfilloux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	check_special(char *s, int i, int j, t_map *map)
{
	...
	if (s[i] == ' ' && j == 0)
	{
		while (map->map[j + diff] && map->map[j + diff][i] == ' ')
			diff ++;
		if (map->map[j + diff][i] && map->map[j + diff][i] != '1')
			return (false);
	}
	else if (s[i] == ' ' && j == (map->height - 1))
	{
		while (map->map[j - diff][i] && map->map[j - diff][i] == ' ')
			diff ++;
		if (map->map[j - diff][i] && map->map[j - diff][i] != '1')
			return (false);
	}
...
int	check_line(char *s, int j, t_map *map, int *spawn)
{
	...
	while (s[i] && s[i] == ' ')
		i ++;
	if (!check_border(map, j))
		return (0);
	else
	{
		while (s[i])
		{
			if (!ft_isspawn(s[i]) && !ft_objs(s[i]) && s[i] != ' ')
				return (-2);
