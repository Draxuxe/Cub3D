...
/*   Updated: 2022/11/04 13:59:00 by lfilloux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static short	check_validity_map(t_cubed *cube)
{
	int	i;
	int	spawn;
	int	j;
	...
	{
		j = check_line(cube->map->map[i], i, cube, &spawn);
		if (j < 1)
			return (j);
		i ++;
	}
